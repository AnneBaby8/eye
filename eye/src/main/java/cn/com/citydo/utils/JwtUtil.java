...
import cn.com.citydo.config.security.JwtConfig;
...
public class JwtUtil {
    ...
    private final long TIME = 30 * 60 * 1000 ;
     ...
     * @return JWT
     */
    public String createJWT(Boolean rememberMe, Long id, String subject, List<Role> roles, Collection<? extends GrantedAuthority> authorities) {
    ...
    public Claims parseJWT(String jwt) {
        try {
            ...
            if (!StrUtil.equals(jwt, redisToken)) {
                throw new BaseException(Status.TOKEN_OUT_OF_CTRL);
            }
            if (expire <= TIME) {
                //验证通过 刷新过期时间
                stringRedisTemplate.expire(Consts.REDIS_JWT_KEY_PREFIX + claims.getSubject(), expire > jwtConfig.getTtl() ? jwtConfig.getRemember() : jwtConfig.getTtl(),
